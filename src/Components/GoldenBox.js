    ...
    return (
        <div>
            <Spring
                to={{
                    opacity: true ? 1 : 0,
                    transform: true ? `transform(0px, 0px)` : "translate(-100px)"
                }}
                from={{ position: "absolute", opacity: 1, transform: props.index === 1 ? `translate(-500px, 0px)` : `translate(0, -500px)` }}
            >
                {prop => (
                    <animated.div style={{
                        ...prop, position: "absolute", width: pos === 1 ? gA : gB, height: pos === 1 ? gA : gB, background: "rgba(255,255,255, 0.9)",
                        top: `${4}px`, left: pos === 1 ? `${4}px` : `${gA + 8}px`, transformOrigin: `${gA + gE + 12}px ${gB + gF + gH + 16}px`
                    }} >
                    </animated.div>
                )}
            </Spring>
